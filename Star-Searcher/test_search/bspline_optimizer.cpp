...
void BsplineOptimizer::optimize() {
  ...
  if (cost_function_ & MINTIME) {
    std::cout << "Iter num: " << iter_num_ << ", time: " << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count()
...
void BsplineOptimizer::combineCost(const std::vector<double>& x, std::vector<double>& grad,
  ...
  comb_time += std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
