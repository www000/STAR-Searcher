...
void FastPlannerManager::initPlanModules() {
  ...
  pp_.dynamic_ = 0;
  pp_.clearance_ = 0.2;
  pp_.local_traj_len_ = 6.0;
  pp_.ctrl_pt_dist = 0.5;
  pp_.bspline_degree_ = 3;
  pp_.min_time_ = true;
  ...
  if (use_geometric_path) {
    ...
    path_finder_->init(edt_environment_);
  }
  ...
  if (use_active_perception) {
    frontier_finder_.reset(new FrontierFinder(edt_environment_));
    heading_planner_.reset(new HeadingPlanner());
    heading_planner_->setMap(sdf_map_);
    visib_util_.reset(new VisibilityUtil);
...
bool FastPlannerManager::checkTrajCollision(double &distance) {
  double t_now = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - local_data_.start_time_).count();
...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d &start_pt,
  ...
  double t_search = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  ...
  double t_opt = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  double tr = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - time_now).count();
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  double tm1 = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  t1 = std::chrono::steady_clock::now();
  ...
  double tm2 = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  t1 = std::chrono::steady_clock::now();
  ...
  double tm3 = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
...
void FastPlannerManager::planYaw(const Eigen::Vector3d &start_yaw) {
  ...
  std::cout << "yaw time: " << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count() << std::endl;
}
...
void FastPlannerManager::planYawExplore(const Eigen::Vector3d &start_yaw,
  ...
  // std::cout << "2: " << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count() << std::endl;
