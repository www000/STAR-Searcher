...
void Astar::init(const EDTEnvironment::Ptr &env) {
  resolution_ = 0.1;
  lambda_heu_ = 10.0;
  max_search_time_ = 0.1;
  max_search_time_unknown_ = 0.1;
...
int Astar::search(const Eigen::Vector3d &start_pt,
  ...
  while (!open_set_.empty()) {
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count() > max_search_time_) {
        ...
        for (double dz = -resolution_ + epsilon_; dz <= resolution_ + epsilon_;
          ...
          if (node_iter == open_set_map_.end()) {
            ...
            if (use_node_num_ == allocate_num_) {
              double diff_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
              std::cout << "[AStar] Run out of node pool. Duration: " << diff_time << std::endl;
              return NO_PATH;
            }
