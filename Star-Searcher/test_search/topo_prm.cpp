...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
  ...
  graph_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  ...
  search_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  ...
  short_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  ...
  prune_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
  ...
  select_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    if (dist <= clearance_) {
      sample_time += std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - t1).count();
      continue;
    }
